...
Flags:
    ...
    -v  :  Path to directory containing sample consensus VCF files (incompatible with -d)
...
while getopts ":d:v:n:e:" option; do
    case "${option}" in
...
if [ $database_dir = 0 ] && [ $vcf_dir = 0 ]; then
    echo "You must specify a data directory containing fastq(.gz) reads (-d) or VCF files (-v)."
    echo "$HELP"
    exit 1
fi
