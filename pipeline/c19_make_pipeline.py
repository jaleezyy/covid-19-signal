...
After the pipeline finishes, you'll see the following files:
   ...
   Iran1/Snakefile
   Iran1/pipeline.yaml                               high-level summary of pipeline configuration and inputs
...
# TODO currently representing Snakefile rules as global string in python code.
# Will move into its own .smk file soon!
snakefile_text =r"""
# Autogenerated by c19_make_pipeline.py -- do not edit!

import yaml

# TODO: error-checking on contents of this file
y = yaml.load(open('pipeline.yaml'))

rule all:
    input:
        lambda wildcards: [ f"fastq_sorted/{y['prefix']}_R{r}_fastqc.html" for r in [1,2] ]

rule concat_and_sort:
    output:
        'fastq_sorted/Iran1_R{r}.fastq.gz'
    input:
        lambda wildcards: y[f'input_fastq_files_R{wildcards.r}']
    shell:
...
class Pipeline:
    ...
    def write_pipeline_yaml(self):
        """Writes {pipeline_output_dir}/pipeline.yaml."""
        
        filename = os.path.join(self.outdir, 'pipeline.yaml')
        print(f"Writing {filename}")

        with open(filename,'w') as f:
            print("# Autogenerated by c19_make_pipeline.py -- do not edit!", file=f)
            print("# This file contains a high-level summary of pipeline configuration and inputs.", file=f)
            print('# It is ingested by the Snakefile, and also intended to be human-readable.', file=f)
            print(file=f)
