...
# Lineage calling and partial postprocess only
# signal.py assumes Snakefile_fasta (renamed Snakefile) is in current working directory (i.e., SIGNAL root)
...
def create_parser():
	allowed = {'all': False, 'postprocess': False}

	parser = argparse.ArgumentParser(prog='signal.py', description="This branch of SARS-CoV-2 Illumina GeNome Assembly Line (SIGNAL) aims to take already resolved consensus FASTA sequences in conjunction with a pre-generated statistics file to provide a summary of analysis for local surveillance.")
	parser.add_argument('all', nargs='*',
						help="Run SIGNAL with all associated lineage calling tools. Does not include postprocessing '--configfile' or '--directory' required. The latter will automatically generate a configuration file and sample table. If both provided, then '--configfile' will take priority")
	parser.add_argument('postprocess', nargs='*',
						help="Run SIGNAL 'partial' postprocessing on completed SIGNAL run. '--configfile' is required but will be generated if '--directory' is provided")
	...
	parser.add_argument('-d', '--directory', type=check_directory, default=None,
						help="Path to directory containing generated consensus FASTA file(s). Will be used to generate sample table and configuration file")
...
negative_control_prefix: {opt_tasks[0]}"""
...
if __name__ == '__main__':
	...
	if args.configfile is None:
		...
		if args.neg_prefix is not None:
			...
			neg = [args.neg_prefix]
		write_config_file(run_name, config_file, [neg])
		if args.config_only:
